...
- 词库为内置英文 5 字母单词表（`resources/words.txt`，共 **2723** 词，每行一词、全大写）
- 提交猜测必须在词库中，且不能重复猜测
...
- **任务特点**：词库约 **2723** 个单词，每次猜测需线性查找；字母判定为 5×5 小规模循环
...
本实验词库已扩展至约 **2723** 词，OpenMP/MPI 并行查词相比小词库更有对比空间，但**不以性能加速为主要目标**，重在理解并行原语在游戏逻辑中的嵌入方式。若需量化加速比，可增加查词调用次数或进一步扩充词库后对比串行与并行耗时。
...
2. **词库规模与 OpenMP 加速**  
   当前词库约 2723 词，并行查词比原先数百词时更有观察空间，但单次猜测仍是 O(n) 量级；作为课程实验仍重在理解 `parallel for` 写法，而非追求极致加速。
